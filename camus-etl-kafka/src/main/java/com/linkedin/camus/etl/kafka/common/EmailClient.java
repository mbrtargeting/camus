...
public class EmailClient {
    ...
    public static void setup(Properties properties) {
        ...
        subjectLine = properties
                .getProperty("alert.email.subject", "Camus topic falling behind alert");
        senderEmail = properties.getProperty("alert.email.sender", "camus@linkedin.com");
        shouldSendEmail = properties.getProperty("alert.on.topic.falling.behind", "true")
    ...
    public static void sendEmail(String content) {
        if (!shouldSendEmail) {
            LOGGER.warn(
                    "EmailLogger is disabled (email.send must be set to true) or has not been setup. "
                    +
