 ...
 * <li>{@code etl.output.file.time.partition.mins} - partitions size in minutes, defaults to {@code
 * 60}</li>
 * <li>{@code etl.destination.path.topic.sub.dirformat} - sub-dir format to create under topic dir,
 * defaults
 * to {@code 'hourly'/YYYY/MM/dd/HH}. See {@link org.joda.time.format.DateTimeFormatter} for
 * syntax.</li>
 * <li>{@code etl.destination.path.topic.sub.dirformat.locale} - locale to use for the sub-dir
 * formatting,
...
public class TimeBasedPartitioner extends BaseTimeBasedPartitioner {
    ...
    public void setConf(Configuration conf) {
        if (conf != null) {
            String destPathTopicSubDirFormat = conf
                    .get(ETL_DESTINATION_PATH_TOPIC_SUBDIRFORMAT, DEFAULT_TOPIC_SUB_DIR_FORMAT);
            long partitionDurationMinutes = Long.parseLong(
                    conf.get(ETL_OUTPUT_FILE_TIME_PARTITION_MINS,
                             DEFAULT_PARTITION_DURATION_MINUTES));
            Locale locale = LocaleUtils.toLocale(
                    conf.get(ETL_DESTINATION_PATH_TOPIC_SUBDIRFORMAT_LOCALE, DEFAULT_LOCALE));
