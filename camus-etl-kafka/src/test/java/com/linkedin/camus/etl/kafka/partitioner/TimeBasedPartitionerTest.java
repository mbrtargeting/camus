...
public class TimeBasedPartitionerTest {
    ...
    public void testDefaultConfiguration() throws Exception {
        ...
        long time = new DateTime(2014, 2, 1, 3, 0, 0, 0, DateTimeZone.forID("America/Los_Angeles"))
                .getMillis();
    ...
    public void testPicksUpConfiguration() throws Exception {
        ...
        underTest.setConf(configurationFor(
                "120", "'bi-hourly'/'year='YYYY/'month='MMMM/'day='dd/'hour='H", "nl_NL",
                "Europe/Amsterdam"));

        long time = new DateTime(2014, 2, 1, 3, 0, 0, 0, DateTimeZone.forID("Europe/Amsterdam"))
                .getMillis();
        String partition = underTest.encodePartition(null, etlKeyWithTime(time));
        String path = underTest.generatePartitionedPath(null, "tpc", partition);
    ...
    private Configuration configurationFor(String partitionDurationMinutes, String subDirFormat,
        ...
        if (partitionDurationMinutes != null) {
            conf.set("etl.output.file.time.partition.mins", partitionDurationMinutes);
        }
        if (subDirFormat != null) {
            conf.set("etl.destination.path.topic.sub.dirformat", subDirFormat);
        }
        if (subDirFormatLocale != null) {
            conf.set("etl.destination.path.topic.sub.dirformat.locale", subDirFormatLocale);
        }
        if (dateTimeZone != null) {
            conf.set("etl.default.timezone", dateTimeZone);
        }
        return conf;
    }
