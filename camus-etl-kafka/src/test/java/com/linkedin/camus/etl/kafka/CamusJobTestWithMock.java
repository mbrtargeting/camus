...
public class CamusJobTestWithMock {
    ...
    private static TopicMetadataResponse createTopicMetadataResponseFromBytes(
        ...
        TopicMetadataResponse response = new TopicMetadataResponse(
                kafka.api.TopicMetadataResponse.readFrom(byteBuffer));
        return response;
    }
    ...
    private void setupRegularJob() {
        ...
        EtlInputFormatForUnitTest.consumer = mockSimpleConsumer(metadataResponse, offsetResponse,
                                                                fetchResponse);
    ...
    private OffsetResponse mockOffsetResponse(List<MyMessage> myMessages) {
        ...
        EasyMock.expect(offsetResponse.offsets(EasyMock.anyString(), EasyMock.anyInt()))
                .andReturn(new long[]{myMessages.size()}).times(1);
        // The second call is getEarliestOffset, we set the value to 0
        EasyMock.expect(offsetResponse.offsets(EasyMock.anyString(), EasyMock.anyInt()))
                .andReturn(new long[]{0}).times(1);
    ...
    private FetchResponse mockFetchResponse(List<MyMessage> myMessages) {
        ...
        EasyMock.expect(fetchResponse.messageSet(EasyMock.anyString(), EasyMock.anyInt()))
                .andReturn(messageSet).times(1);
        mocks.add(fetchResponse);
        return fetchResponse;
    }

    private SimpleConsumer mockSimpleConsumer(TopicMetadataResponse metadataResponse,
        ...
        EasyMock.expect(simpleConsumer.getOffsetsBefore((OffsetRequest) EasyMock.anyObject()))
                .andReturn(offsetResponse).anyTimes();
        simpleConsumer.close();
        EasyMock.expectLastCall().andVoid().anyTimes();
        EasyMock.expect(simpleConsumer.clientId()).andReturn(KAFKA_CLIENT_ID).times(1);
        EasyMock.expect(simpleConsumer.fetch((FetchRequest) EasyMock.anyObject()))
                .andReturn(fetchResponse).times(1);
    ...
    private void setupJobFailDueToOffsetRangeCallException() {
        TopicMetadataResponse metadataResponse = mockTopicMetaDataResponse();
        EtlInputFormatForUnitTest.consumerType = EtlInputFormatForUnitTest.ConsumerType.MOCK;
        EtlInputFormatForUnitTest.consumer = mockConsumerThrowsExceptionForOffsetRangeCall(
                metadataResponse);
        EasyMock.replay(mocks.toArray());
    }

    private SimpleConsumer mockConsumerThrowsExceptionForOffsetRangeCall(
            TopicMetadataResponse metadataResponse) {
        SimpleConsumer simpleConsumer = EasyMock.createMock(SimpleConsumer.class);
        mocks.add(simpleConsumer);
        EasyMock.expect(simpleConsumer.send((TopicMetadataRequest) EasyMock.anyObject()))
                .andReturn(metadataResponse).times(1);
    ...
    private void setupJobWithSkippedMsgSchemaNotFound() {
        EtlInputFormatForUnitTest.consumerType = EtlInputFormatForUnitTest.ConsumerType.MOCK;
        EtlInputFormatForUnitTest.recordReaderClass
                = EtlInputFormatForUnitTest.RecordReaderClass.TEST;
        EtlRecordReaderForUnitTest.decoderType
                = EtlRecordReaderForUnitTest.DecoderType.SCHEMA_NOT_FOUND_30_PERCENT;
        TopicMetadataResponse metadataResponse = mockTopicMetaDataResponse();
        List<MyMessage> myMessages = messagesWritten.get(TOPIC_1);
        OffsetResponse offsetResponse = mockOffsetResponse(myMessages);
        FetchResponse fetchResponse = mockFetchResponse(myMessages);
    ...
    private void setupJobWithSkippedMsgOther() {
        ...
        EtlInputFormatForUnitTest.recordReaderClass
                = EtlInputFormatForUnitTest.RecordReaderClass.TEST;
        EtlRecordReaderForUnitTest.decoderType
                = EtlRecordReaderForUnitTest.DecoderType.OTHER_30_PERCENT;
        TopicMetadataResponse metadataResponse = mockTopicMetaDataResponse();
        List<MyMessage> myMessages = messagesWritten.get(TOPIC_1);
        OffsetResponse offsetResponse = mockOffsetResponse(myMessages);
        FetchResponse fetchResponse = mockFetchResponse(myMessages);
    ...
    public void testJobFailOffsetTooEarly() throws Exception {
        EtlInputFormatForUnitTest.camusRequestType
                = EtlInputFormatForUnitTest.CamusRequestType.MOCK_OFFSET_TOO_EARLY;
        setupRegularJob();
    ...
    public void testJobFailOffsetTooLate() throws Exception {
        EtlInputFormatForUnitTest.camusRequestType
                = EtlInputFormatForUnitTest.CamusRequestType.MOCK_OFFSET_TOO_LATE;
        setupRegularJob();
        props.setProperty(EtlInputFormat.KAFKA_MOVE_TO_EARLIEST_OFFSET, Boolean.FALSE.toString());
        job = new CamusJob(props);
        job.run(EtlInputFormatForUnitTest.class, EtlMultiOutputFormat.class);
    ...
    public void testJobFailDueToPublishCountsException() throws Exception {
        ...
        props.setProperty(CamusJob.CAMUS_MESSAGE_ENCODER_CLASS,
                          "com.linkedin.camus.etl.kafka.coders.EncoderForUnitTest");
        props.setProperty(CamusJob.ETL_COUNTS_CLASS,
                          "com.linkedin.camus.etl.kafka.common.EtlCountsForUnitTest");
        job = new CamusJob(props);
        job.run(EtlInputFormatForUnitTest.class, EtlMultiOutputFormat.class);
    }
    ...
    private void setupJobPublishCountsThirdTimeSucceed() {
        ...
        EtlInputFormatForUnitTest.consumer = mockSimpleConsumer(metadataResponse, offsetResponse,
                                                                fetchResponse);
        EtlCountsForUnitTest.producerType
                = EtlCountsForUnitTest.ProducerType.SEND_SUCCEED_THIRD_TIME;
        EasyMock.replay(mocks.toArray());
    }
    ...
    private void assertCamusContains(String topic, List<MyMessage> messages)
    ...
    private List<MyMessage> readMessages(String topic)
